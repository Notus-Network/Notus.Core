            ...
            if (ResetBlocksIfNonValid == true)
            {
                ...
                foreach (string fileName in ZipFileList)
                {
                    ...
                    catch (Exception err)
                    {
                        ...
                        Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Error Text [7abc63]: " + err.Message);
                    }
                }
            }
            return (false, null);
                        ...
                        else
                        {
                            ...
                            if (zipEntry != null)
                            {
                                ...
                                using (StreamReader zipEntryStream = new StreamReader(zipEntry.Open()))
                                {
                                    ...
                                    {
                                        Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Error Text [235abc]: " + err.Message);
                                    }
                                }
                            }
                        }
            ...
            if (SmallestBlockHeight > 1)
            {
                ...
                while (exitInnerLoop == false)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Getting Block Row No : " + SmallestBlockHeight.ToString());
        ...
        private (string, string) GetBlockSign(Int64 BlockRowNo)
        {
            ...
            while (exitInnerLoop == false)
            {
                for (int a = 0; a < Notus.Variable.Constant.ListMainNodeIp.Count && exitInnerLoop == false; a++)
                {
                    ...
                    {
                        Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Error Text [96a3c2]: " + err.Message);
                        Thread.Sleep(5000);
                    }
                }
            }
        ...
        private void StoreBlockWithRowNo(Int64 BlockRowNo)
        {
            ...
            while (exitInnerLoop == false)
            {
                for (int a = 0; a < Notus.Variable.Constant.ListMainNodeIp.Count && exitInnerLoop == false; a++)
                {
                    ...
                    {
                        Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Error Text [5a6e84]: " + err.Message);
                        Notus.Toolbox.Print.Basic(Obj_Settings.DebugMode, "Income Text [5a6e84]: " + MainResultStr);
                    }
                }
            }
        }
