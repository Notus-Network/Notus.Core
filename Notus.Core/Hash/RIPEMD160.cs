...
namespace Notus.HashLib
{
    public class RIPEMD160
    {
        ...
        {
            return
                (System.Convert.ToUInt32(buffer[3 + offset]) << 24) |
                (System.Convert.ToUInt32(buffer[2 + offset]) << 16) |
                (System.Convert.ToUInt32(buffer[1 + offset]) << 8) |
                (System.Convert.ToUInt32(buffer[0 + offset]));
        }
        ...
        static private void MDinit(ref UInt32[] MDbuf)
        {
        ...
        static private void compress(ref UInt32[] MDbuf, UInt32[] X)
        {
            UInt32 aa = MDbuf[0];
            UInt32 bb = MDbuf[1];
            UInt32 cc = MDbuf[2];
        ...
        static private void MDfinish(ref UInt32[] MDbuf, byte[] strptr, long index, UInt32 lswlen, UInt32 mswlen)
        {
            var X = Enumerable.Repeat((UInt32)0, 16).ToArray();
            for (var i = 0; i < (lswlen & 63); i++)
            {
                X[i >> 2] ^= System.Convert.ToUInt32(strptr[i + index]) << (8 * (i & 3));
            }
            X[(lswlen >> 2) & 15] ^= (UInt32)1 << System.Convert.ToInt32(8 * (lswlen & 3) + 7);
        ...
        public string SignWithHashMethod(string keyText, string input)
        {
            ...
            return ComputeHash(k_opad + ComputeHash(k_ipad + input).ToLower()).ToLower();
        }
        ...
        {
            MDfinish(ref MDbuf, UnhashedBuffer, 0, System.Convert.ToUInt32(HashedLength), 0);

            var result = new byte[RMDsize / 8];

            for (var i = 0; i < RMDsize / 8; i += 4)
            {
                result[i] = System.Convert.ToByte(MDbuf[i >> 2] & 0xFF);
                result[i + 1] = System.Convert.ToByte((MDbuf[i >> 2] >> 8) & 0xFF);
                result[i + 2] = System.Convert.ToByte((MDbuf[i >> 2] >> 16) & 0xFF);
                result[i + 3] = System.Convert.ToByte((MDbuf[i >> 2] >> 24) & 0xFF);
            }
