...
namespace Notus.HashLib
{
    ...
    {
        const int R = 10;
        const int DIGEST_LONG = 8;
        const int DIGEST_BYTES = 64;
        const int DIGEST_BITS = 512;
        ...
        readonly byte[] bitlength = new byte[LENGTH_BYTES];
        readonly byte[] buffer = new byte[WBLOCK_BYTES];
        readonly ulong[] hash = new ulong[WBLOCK_LONG];
        int buffer_bits;
        int buffer_pos;
        ...
        void NESSIE_Add(byte[] src, int _src_pos, int _src_len)
        {
            ...
            var buf = buffer;
            var buf_rem = buffer_bits & 7;
            var buf_bits = buffer_bits;
            var buf_pos = buffer_pos;
        ...
        void NESSIE_Finalize(byte[] result)
        {
            ...
            buf[buf_pos] |= (byte)(0x80U >> (buf_bits & 7));
            buf_pos++;

            if (buf_pos > WBLOCK_BYTES - LENGTH_BYTES)
            {

                if (buf_pos < WBLOCK_BYTES)
                    Array.Clear(buf, buf_pos, WBLOCK_BYTES - buf_pos);
            ...
            if (buf_pos < WBLOCK_BYTES - LENGTH_BYTES)
                Array.Clear(buf, buf_pos, (WBLOCK_BYTES - LENGTH_BYTES) - buf_pos);

            buf_pos = WBLOCK_BYTES - LENGTH_BYTES;

            // Append length
            Array.Copy(bit_len, 0, buf, WBLOCK_BYTES - LENGTH_BYTES, LENGTH_BYTES);

            // Process block
            Process();

            for (i = 0; i < DIGEST_LONG; i++)
            {

                digest[offset + 0] = (byte)(hash[i] >> 56);
                digest[offset + 1] = (byte)(hash[i] >> 48);
                digest[offset + 2] = (byte)(hash[i] >> 40);
                digest[offset + 3] = (byte)(hash[i] >> 32);
                digest[offset + 4] = (byte)(hash[i] >> 24);
                digest[offset + 5] = (byte)(hash[i] >> 16);
                digest[offset + 6] = (byte)(hash[i] >> 8);
                digest[offset + 7] = (byte)(hash[i]);
        ...
        {
            NESSIE_Init();
            NESSIE_Add(inputArr, 0, inputArr.Length);
            byte[] buffer = new byte[DIGEST_BYTES];
            NESSIE_Finalize(buffer);
            return buffer;
        }

        readonly static ulong[] C0 = new ulong[256] {
        ...
        readonly static ulong[] C1 = new ulong[256] {
        ...
        readonly static ulong[] C2 = new ulong[256] {
        ...
        readonly static ulong[] C3 = new ulong[256] {
        ...
        readonly static ulong[] C4 = new ulong[256] {
        ...
        readonly static ulong[] C5 = new ulong[256] {
        ...
        readonly static ulong[] C6 = new ulong[256] {
        ...
        readonly static ulong[] C7 = new ulong[256] {
        ...
        readonly static ulong[] RC = new ulong[R + 1] {
            0x0000000000000000,
            0x1823c6e887b8014f,
            0x36a6d2f5796f9152,
            0x60bc9b8ea30c7b35,
