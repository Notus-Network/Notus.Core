...
namespace Notus.Communication
{
    public class Http : IDisposable
    {
        ...
        private byte[] IncomeTextFunction(byte[] incomeArray, System.Net.IPEndPoint RemoteEndPoint, System.Net.IPEndPoint LocalEndPoint)
        {
            ...
            {
                Notus.Toolbox.Print.Basic(DebugModeActivated, "Url Doesn't Exist -> " + incomeData.Url);
            }
            else
            {
                Notus.Toolbox.Print.Basic(DebugModeActivated, "Url Call : " + incomeData.RawUrl);
        ...
        public void Start(IPAddress NodeIPAddress, int PortNo)
        {
            ...
            try
            {
                ...
                {
                    Notus.Toolbox.Print.Basic(DebugModeActivated, "Error Code : " + errorCode.ToString());
                    Notus.Toolbox.Print.Basic(DebugModeActivated, "Error Text : " + errorText);
                });
                ListenerObj.OnReceive(IncomeTextFunction);
            }
            catch (Exception e)
            {
                Notus.Toolbox.Print.Basic(DebugModeActivated, "An Exception Occurred while Listening :" + e.ToString());
            }
        }
        ...
        private Notus.Variable.Struct.HttpRequestDetails ParseString(byte[] rawArray)
        {
            ...
            */
            ...
            Notus.Toolbox.Print.Basic(DebugModeActivated, urlLine);
